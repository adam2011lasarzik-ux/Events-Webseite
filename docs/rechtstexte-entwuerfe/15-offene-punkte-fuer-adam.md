...
| **4.6a** | ⚠️ **Neu, kurze Rückfrage.** Soll der Vorbehalt „…soweit keine andere Rechtsgrundlage besteht" aus dem an den Verbraucher gerichteten Einwilligungstext **gestrichen** werden (Empfehlung — für reine Werbeaufnahmen gibt es realistisch keine andere Grundlage, und der Satz macht die Widerrufsfolge unüberprüfbar), oder gibt es einen konkreten Fall, für den du ihn brauchst? |
...
- **B-15** — Aus Entscheidung 4.7 folgt ein **neues, bewusst grobes Altersmerkmal je Teilnehmer** (`UNTER_14`, `VIERZEHN_BIS_17`, `AB_18`), angegeben von der buchenden erwachsenen Person, ohne Altersprüfung. Belegt, warum das Vorhandene nicht genügt: `Participant.typ` (`SCHUELER`/`ERWACHSENER`) ist eine **Preiskategorie**, keine Altersangabe — ein Schüler kann 19 sein, und seit Schritt S lässt sich die Schülerkategorie ganz abschalten. Das Feld `Participant.geburtsjahr` existiert im Schema, wird aber **nirgends erhoben** (einziger Treffer in `lib/`, `components/`, `app/`, `content/`: `lib/anonymisieren.ts:60`, das es beim Löschen auf `null` setzt) und ist an der 14er-Grenze ohnehin mehrdeutig. Dazu: Check-in-Liste weist bei 14–17 die fehlende zweite Zustimmung aus und lässt sie dort festhalten; der Status „eingewilligt" entsteht erst, wenn **beide** Zustimmungen vorliegen. Totes Feld `geburtsjahr` entfernen oder ausdrücklich als „nicht erhoben" dokumentieren.
